"""build/branding/brand_check.py — post-build brand verifier + endpoint scan.
...
2. `--scan`: endpoint-deny scan over THIS repo's build surface only
   (build/, ci/, DEPS, .github/workflows) — NOT docs/ (which legitimately
...
SCAN_DIRS = ["build", "scripts", "ci", "DEPS", ".github/workflows"]
...
def main() -> None:
    parser = argparse.ArgumentParser(prog="build/branding/brand_check.py",
