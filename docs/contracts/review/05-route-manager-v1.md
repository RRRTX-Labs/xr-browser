# Review packet: xr.mojom.RouteManager: Bind/Unbind/Status/LoseAllFailClosed v1 (§1.11)

- **Contract id:** route-manager  **Version:** 1  **Freeze status:** REVIEW-COMPLETE
- **Ratification:** PENDING (HG-26; human sign-off below is pending)

## Interface inventory
L6: fail-closed on route loss; recovery gated on user_ack.
...
xr-core/mojom/route_manager.mojom, fakes/route_manager.py, vectors/route-manager-v1.json
...
- [ ] fuzz target required before integration — target: `fuzz_route_manager` (P9; declaration only) — sign-off: __PENDING (human)__
