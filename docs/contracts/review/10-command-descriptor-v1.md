# Review packet: CommandRegistry descriptor (schema) v1 (§1.11)

- **Contract id:** command-descriptor  **Version:** 1  **Freeze status:** REVIEW-COMPLETE
- **Ratification:** PENDING (HG-26; human sign-off below is pending)

## Interface inventory
§10: danger-class on every command; declarative handler only.
...
command-descriptor-v1.md, command-descriptor-v1.schema.json
...
- [ ] fuzz target required before integration — target: `fuzz_command_descriptor` (P9; declaration only) — sign-off: __PENDING (human)__
