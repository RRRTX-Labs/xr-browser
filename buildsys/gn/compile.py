"""buildsys/gn/compile.py — `./build compile`: run ninja on a target.
...
def main() -> None:
    parser = argparse.ArgumentParser(prog="buildsys/gn/compile.py",
    ...
    def run_compile() -> int:
        ...
        if not (out_dir / "build.ninja").exists() and not mock_enabled():
            raise ToolError(f"{out_dir}/build.ninja missing; run ./build gen first")
        if mock_enabled():
