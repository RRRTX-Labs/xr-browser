...
`buildsys/farm/ccache-setup.sh` provisions `~/.ccache` with a max-size policy:
...
present; `./build gen` adds it when `ccache` is on PATH — see
`buildsys/gn/gen.py`). Never vendored; the binary is a system/brew/winget
install.
...
`direct_mode` sloppiness), and `./build compile` re-runs `ccache -s` and fails
