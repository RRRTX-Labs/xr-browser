"""tools/update_vectors_families.py — the golden-vector case FAMILIES
(P11-T0-e split of tools/gen_update_vectors.py by the touched-file size law,
<=380 lines, split by responsibility).

One builder per method family of docs/contracts/vectors/update-v1.json:
verify axes, epoch-apply, cohort+backoff, about-state. The orchestrator
(gen_update_vectors.py) owns the CLI, the add/V closures and the document
assembly; the builders are verbatim moves of the case-construction blocks —
same order, same ids, same expectations — so `--check` regenerates the
committed vectors byte-identical (the proof this split changed nothing).
Fixture primitives (envelope/stub_sig/EPOCH/KEYS) come from
tools/update_vectors_kit.py, the stub-signature law's single definition.
"""
...
from update_vectors_kit import EPOCH, KEYS, envelope, stub_sig
...
def build_verify_family(V, add, good, verify_args) -> None:
    """The verify-axis cases: accept paths, TLS independence, unknown-field
    denials at every level, protocol/shape, oversize+prefix, app/offer,
    version law, package law, signature law, epoch law, monotonicity+replay,
    channel refusals and malformed frames."""
    # ---- accept paths (4) ----
...
def build_epoch_apply_family(add) -> None:
    """The epoch-apply surface: valid notice, stale-ignored, bad signature,
    unknown field."""
...
def build_cohort_backoff_family(add) -> None:
    """Cohort determinism/separation/boundaries + the backoff law (6h floor,
    doubling, 48h cap, monotonic clock never goes backwards)."""
...
def build_about_state_family(add) -> None:
    """The about-state machine: every transition + the refused-terminal law
    + the invalid-transition refusal."""
